...
#include "ShiftedLCD.h" // Arduino Library for 74HC595 Shift Register using SPI
...
void setup() {
  ...
  lcd.print("Access Controlv3");
  ShowReaderDetails();  // Show details of PCD - MFRC522 Card Reader details

  //Wipe Code - If the Button (wipeB) Pressed while setup run (powered on) it wipes EEPROM
  if (digitalRead(wipeB) == LOW) {  // when button pressed pin should get low, button connected to ground
    lcd.clear();
    lcd.print("WipeB is Pressed");
    ...
    delay(3000);
    lcd.clear();
    lcd.print("EEPROM will be");
    lcd.setCursor(0,1);
    lcd.print("Wiped in 15 sec");
    delay(12000);                        // Give user enough time to cancel operation
